...
function WalletItem({
  ...
  return (
    <Flex
      ...
      rounded="md"
      _hover={
        selectable ? { bg: isLight ? '#8C6EEF' : '#133837', color: isLight ? colors.textRevertPrimary : colors.textPrimary } : undefined
      }
      bg="#174644"//{colors.backgroundDark}
      py={3}
      px={3}
      pl={[9, 3]}
    ...
    >
      ...
      {wallet.adapter.name === 'Phantom' && (
        <HStack gap={1} backgroundColor={colors.selectInactive} px={2} py={1} borderRadius="8px">
          <Text fontSize="12px" color="#42EDE1">
      ...
      {wallet.adapter.name === 'Solflare' && (
        <HStack gap={1} backgroundColor={colors.selectInactive} px={2} py={1} borderRadius="8px">
          ...
          {/* <QuestionToolTip label={t('wallet_connect_panel.auto_approve_tip_solflare')} iconProps={{ color: "#42EDE1" }} /> */}
        </HStack>
      )}
    </Flex>
  )
