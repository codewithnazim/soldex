...
import create, { StateCreator, Mutate, StoreApi } from 'zustand'
import { devtools } from 'zustand/middleware'
import { immer } from 'zustand/middleware/immer'
...
// Correct StateCreator signature with Middlewares
const createStore = <T>(fn: StateCreator<T, [['zustand/devtools', never], ['zustand/immer', never]]>, name?: string) => {
  // Create the store with middleware
  const store = create<T>()(devtools(immer(fn), name ? { name } : undefined))

  const initState = store.getState()

  // Store reseter function for resetting the state to its initial values
  storeResetter.push({
    name,
    reset: (replaceState?: Record<string, any>) => {
      store.setState({ ...initState, ...(replaceState || {}) }, true, { type: 'reset' })
